...
> **Claude Code / WorkBuddy / Codex CLI / ChatGPT** 等多平台兼容。
>
...
本技能包为 **Claude Code / WorkBuddy / Codex CLI / ChatGPT** 等 AI Agent 提供了一套标准化的网络安全作战框架。它不是一句简单的"你是安全专家"，而是一套可执行的**协议级规范**。
...
├── SKILL.md                     # 🎯 Agent 指令（压缩版，~250 行）
├── README.md                    # 👤 本文件 — 人类使用手册
├── LICENSE                      # MIT 许可证
├── CHANGELOG.md                 # 版本历史
├── .gitignore                   # Git 忽略规则
│
...
```bash
# 将目录放入 Claude Code skills/ 路径
~/.claude/skills/full-stack-cybersecurity-expert/
```

### 在 WorkBuddy 中使用

```bash
# 将目录放入 WorkBuddy skills/ 路径
~/.workbuddy/skills/full-stack-cybersecurity-expert/
```
...
1. **Fork** 本仓库
2. 创建分支：`add/skill-enhancement` 或 `fix/skill-issue`
3. 提交 PR 时附上修改理由和效果说明
4. 确保不降低 Agent 执行效果

### PR 规范

- **references/ 修改**：确保与 SKILL.md 中的引用路径一致；技术信息（CVE/Event ID/命令语法）需附来源
- **examples/ 修改**：Phase 命名须与 `references/mode-definitions.md` 阶段流对齐
- **SKILL.md 修改**：保持压缩版设计（≤250 行核心指令），深度内容放 references/
- **安全红线**：不得降低 Human-in-the-Loop 确认要求；不得移除脱敏规则
